...
let htmlElem = `<div class="inputForm">
                    <h1>TODO Cat List</h1>
                    <p id="allert"></p>
                    <form class="form">
                        <input id="inputTask" placeholder="Add New Action" required>
                        <button id="taskButton"><i class="material-icons">add</i></button>
                    </form>
                </div>
                <div id="list"></div>
                <div id="footer">
...
function addTask(e) {
    ...
    if (tasks.length < constants.ten) {
        ...
        let inputForm = document.getElementById('inputTask');
...
function deleteTask(index) {
    tasks.splice(index, constants.one);
...
function updateList() {
    ...
    listElem.querySelectorAll('.deleteButton').forEach((btn, index) => {
    ...
    function handleDragStart(e) {
        ...
        let newInd = +e.target.getAttribute('data-task-index') + constants.one;
    ...
    function handleDrop(e) {
        ...
        let oldInd = +e.target.getAttribute('data-task-index') + constants.one;
        let newInd = +e.target.getAttribute('data-task-index');
        tasks.splice(newInd, constants.zero, tasks.splice(oldInd, constants.one)[constants.zero]);
        return false;
    }
